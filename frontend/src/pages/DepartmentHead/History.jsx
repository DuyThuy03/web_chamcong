...
const HistoryPage = () => {
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            const payload = {
                work_unit: parseFloat(editForm.work_unit),
                is_valid: editForm.is_valid,
                note: editForm.note
            };

            const updated = await attendanceService.updateAttendance(editingRecord.id, payload);
            ...
            <div className="bg-[var(--bg-secondary)] rounded-2xl shadow-md border border-[var(--border-color)] p-5 transition-colors duration-300">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <div className="space-y-1.5">
                        <label className="text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wide ml-1 [&::-webkit-calendar-picker-indicator]:opacity-0 [&::-webkit-calendar-picker-indicator]:absolute [&::-webkit-calendar-picker-indicator]:right-0 [&::-webkit-calendar-picker-indicator]:w-full [&::-webkit-calendar-picker-indicator]:h-full [&::-webkit-calendar-picker-indicator]:cursor-pointer">Từ ngày</label>
                        <div className="relative">
                            ...
                            <Calendar 
                                            size={18} 
                                            className="absolute right-3 top-1/2 transform -translate-y-1/2 text-[var(--text-secondary)] group-focus-within:text-[var(--accent-color)] pointer-events-none transition-colors" 
                                        />
                        </div>
                    </div>
                                        ...
                                        <tr key={r.id} className="hover:bg-[var(--accent-color)]/10 even:bg-black/50 [.light_&]:even:bg-gray-50 border-b-2 border-slate-600 [.light_&]:border-slate-200 transition-colors duration-150 group">
                                            ...
                                            <td className="px-6 py-4 text-sm border-r border-slate-600 [.light_&]:border-slate-200">
                                                 ...
                                                 <button
                                                    ...
                                                    className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-blue-500 text-white hover:bg-blue-600 hover:scale-115 transition-all font-bold text-xs shadow-md active:scale-90 ml-2"
                                                 >
                                                    <Edit size={14} />
                                                    Sửa
                                                 </button>
                            ...
                            <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
                                <div className="relative w-full max-w-md bg-[var(--bg-secondary)] rounded-2xl border border-[var(--border-color)] shadow-2xl flex flex-col max-h-[90vh]">
                                    ...
                                    <div className="p-4 border-t border-[var(--border-color)] flex justify-end gap-2">
                                        <button onClick={() => setIsEditModalOpen(false)} className="px-4 py-2 rounded-lg border border-[var(--border-color)] text-[var(--text-secondary)] hover:bg-[var(--bg-primary)] hover:scale-110 transition-all active:scale-95 hover:shadow-lg">
                                            Hủy
                                        </button>
                                        <button onClick={handleSaveEdit} className="px-4 py-2 rounded-lg bg-[var(--accent-color)] text-black hover:bg-[var(--accent-color)]/90 font-medium hover:scale-110 transition-all active:scale-95 hover:shadow-lg">
                                            Lưu thay đổi
                                        </button>
                                    </div>
                                </div>
