...
const AnalyticsPage = () => {
  ...
  const fetchAnalyticsData = async () => {
    ...
    try {
        ...
        console.log("Dữ liệu chấm công",summaryRes);    
        const leavesRes = await api.get("/leaves?limit=500");       
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[var(--bg-secondary)] p-4 rounded-lg border border-[var(--border-color)] shadow-sm">
        ...
        <div className="relative group">
            <input 
                ...
                className="pl-4 pr-10 py-2 bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-md text-[var(--text-primary)] focus:ring-2 focus:ring-[var(--accent-color)] outline-none transition-all shadow-sm appearance-none [&::-webkit-calendar-picker-indicator]:opacity-0 [&::-webkit-calendar-picker-indicator]:absolute [&::-webkit-calendar-picker-indicator]:right-0 [&::-webkit-calendar-picker-indicator]:w-full [&::-webkit-calendar-picker-indicator]:h-full [&::-webkit-calendar-picker-indicator]:cursor-pointer"
            />
            <Calendar 
                size={18} 
                     ...
                     <div className="h-72">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={attendanceTrend}>
                                ...
                                <Legend />
                                <Area type="monotone" dataKey="present" name="Đi làm" stroke="#3b82f6" fillOpacity={1} fill="url(#colorPresent)" />
                                <Area type="monotone" dataKey="late" name="Đi muộn" stroke="#f59e0b" fillOpacity={1} fill="url(#colorLate)" />
                                <Area type="monotone" dataKey="leave" name="Nghỉ phép" stroke="#10b981" fillOpacity={1} fill="url(#colorLeave)" />
                            </AreaChart>
                        </ResponsiveContainer>
                     </div>
                </div>
