...
const DepartmentHeadDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch more leaves to ensure we have the full queue (limit 100) and specifically pending ones
      const [usersRes, attendanceRes, leaveRes] = await Promise.all([
        api.get("/users"),
        api.get("/manager/attendance/today"),
        api.get("/leaves?status=CHO_DUYET&limit=100"), 
      ]);
      ...
      if (leaveRes.data.success) {
        ...
        // Optional: filter again client side to be sure, though API handles it
        pendingLeavesList = pendingLeavesList.filter(l => l.status === "CHO_DUYET");
      }
      ...
      setPendingLeaveRequests(pendingLeavesList); // Store ALL pending requests
    } catch (error) {
  ...
  return (
    <div className="min-h-[100dvh] bg-[var(--bg-primary)] px-4 pt-[calc(1rem_+_env(safe-area-inset-top))] pb-[calc(1rem_+_env(safe-area-inset-bottom))] space-y-4 font-sans text-[var(--text-primary)] transition-colors duration-200">
      {/* 1. Dashboard Header */}
      <div className="bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-lg px-4 md:px-6 py-4 shadow-sm transition-colors duration-300">
                  ...
                  >
                    ...
                    <div className="flex gap-2 pl-[42px]">
                      <button
                        ...
                        className="flex-1 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-[11px] font-bold rounded-md shadow-sm transition-all flex items-center justify-center gap-1 hover:shadow-lg hover:scale-105 hover:-translate-y-0.5 active:scale-90"
                      >
                        <CheckCircle2 size={12} /> Duyệt
                      </button>
                      <button
                        onClick={() => handleRejectLeave(leave.id)}
                        className="flex-1 py-1.5 bg-[var(--bg-secondary)] border border-[var(--border-color)] text-[var(--text-secondary)] hover:bg-rose-500/10 hover:text-rose-500 hover:border-rose-500/20 text-[11px] font-bold rounded-md transition-all flex items-center justify-center gap-1 hover:shadow-lg hover:scale-105 hover:-translate-y-0.5 active:scale-90"
                      >
