...
import {
  FileText, // Changed to match Manager
  Calendar,
  CheckCircle, // Changed to match Manager
  XCircle,
  Clock,
  Filter,
  AlertCircle,
...
const getStatusIcon = (status) => {
  ...
  if (loading) {
    // Render loading INSIDE the main container structure to prevent flashing
    // We will handle this in the main return
  }
                ...
                {leaveRequests.map((r) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-[var(--bg-primary)] border border-[var(--border-color)] flex items-center justify-center text-[var(--accent-color)] font-bold shadow-sm rounded-xl">
                          {r.user_name?.charAt(0).toUpperCase()}
                        </div>
                        <div>
                    ...
                    {r.status === "CHO_DUYET" && (
                      <div className="flex gap-2 pt-2">
                        <button
                          onClick={() => handleApproveLeave(r.id)}
                          className="flex-1 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold transition-all shadow-sm flex items-center justify-center gap-2 rounded-xl hover:shadow-lg hover:scale-105 active:scale-90 active:shadow-inner"
                        >
                          <CheckCircle size={16} />
                          Duyệt đơn
                        </button>
                        <button
                          onClick={() => handleRejectLeave(r.id)}
                          className="flex-1 py-2.5 bg-[var(--bg-primary)] border border-[var(--border-color)] text-[var(--text-secondary)] hover:bg-rose-500 hover:text-white hover:border-rose-500 text-xs font-bold transition-all flex items-center justify-center gap-2 rounded-xl hover:shadow-lg hover:scale-105 active:scale-90 active:shadow-inner"
                        >
                          <XCircle size={16} />
                          Từ chối
                        </button>
                    ...
                    {(r.status === "DA_HUY" ||
                      ...
                      <button
                        onClick={() => handleDelete(r.id)}
                        className="w-full py-2.5 mt-3 bg-rose-500/10 text-rose-500 hover:bg-rose-500 hover:text-white text-xs font-bold transition-all shadow-sm rounded-xl hover:shadow-md hover:scale-105 active:scale-95 active:shadow-inner"
                      >
                        Xóa đơn
                      </button>
                    )}
